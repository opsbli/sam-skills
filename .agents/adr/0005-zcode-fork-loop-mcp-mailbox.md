...
- **Auth split (verified live).** The desktop app routes plan providers (BigModel/Z.AI coding plans, no inline apiKey) through a desktop-local `endpointRoutingPort` proxy that injects plan auth — a channel the bare CLI does not have. `zcode login` only provisions the Z.AI plan's key. The headless runner therefore does **not** need any ZCode plan at all: it runs on whatever explicit provider the user already configures, including third-party OpenAI-compatible endpoints. Verified end-to-end on this machine: pointing `~/.zcode/cli/config.json` at the user's own provider — `provider.<id>.options.{baseURL, apiKey}` plus `model.main: "<provider>/<model-id>"` (the string form is required; an object form is accepted by the schema but its `kind`/`baseURL` fields are ignored by the ref parser) — makes `zcode -p` complete both a plain reply and a tool-using turn (Read) successfully. The earlier provider-1113 finding stands as the negative case: a plan provider whose key has no balance fails the same way the desktop would.
