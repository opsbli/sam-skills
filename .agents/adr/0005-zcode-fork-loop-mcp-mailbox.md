...
- Headless mode refuses to start without an explicit model provider in `~/.zcode/cli/config.json` (`model.main = {provider, model, kind, baseURL, apiKey?}`); `--settings` appears in help text but is not wired in the root parser (advertising/implementation mismatch — do not use it).
- **Auth split (verified live).** `zcode login` performs the **Z.AI plan** OAuth (`chat.z.ai` → `api.z.ai/api/anthropic`), exchanges the access token for a plan API key, and writes it into the CLI config. The desktop app in this install runs on the **BigModel coding plan** (`open.bigmodel.cn/api/anthropic`, provider `builtin:bigmodel-coding-plan`, no inline apiKey) and routes its requests through a desktop-local `endpointRoutingPort` proxy that injects plan auth — a channel the bare CLI does not have. Consequence: headless spawn works only against a plan the CLI's own written key can pay for. Verified empirically: `zai/glm-5.1` and `zai/glm-4.7` return provider error `1113` (insufficient balance) with the login-written key, while the same account's desktop session runs GLM-5.3 on the BigModel plan. The spawn path therefore requires one of: a funded Z.AI plan, an explicit `apiKey` in the CLI config for a paid endpoint, or `FORK_LOOP_ZCODE_CMD` pointing at a CLI wrapper that injects the plan credential. This is a billing/topology boundary, not a defect in the transport.
