...
- **[project-standards](./project-standards/SKILL.md)** — *(fork addition)* Generate, update, or audit `docs/agents/project-standards.md`: exploration agents find the facts, humans confirm the standards, execution skills enforce the document.
- **[harvest](./harvest/SKILL.md)** — *(fork addition)* Turn the pipeline's telemetry ledgers (`docs/metrics.md`, `docs/skill-friction-log.md`) and eval results into concrete skill-revision proposals. Nothing edits a skill file directly — proposals wait for explicit human approval.
...
- **[spec-executor](./spec-executor/SKILL.md)** — *(fork addition)* Implement one approved `SPEC READY` in a forked thread against its fixed point and permission boundary, then return a `SPEC EXECUTION RECEIPT` to the planning thread.
