...
| 项目 | 状态 | 改动 | 验证证据 |
|---|---|---|---|
| **🟢 #42 锁无 PID 存活校验** | ✅ | `task.json` 记录持有者的 **pid**；`acquireLock` 在时间判断**之前**用 `process.kill(pid, 0)` 探测——`ESRCH` 即死、`EPERM` 即活但非我所有。持有者进程已死 → **立即**破锁，不等 6h | 新测试：用 spawn+reap 造出一个**保证已死**的 pid（而非猜越界 id——POSIX 报 `ESRCH`、Windows 报 `EINVAL`，平台相关），断言 spawn 成功且新执行持有锁。**变异测试**：去掉探测 → 该条立刻变红（26/27） |

**向后兼容的边界（写明）**：没有记录 pid 的旧锁（旧版本写入的）**不做**存活探测、照走时间扫描——用"猜"来决定是否破掉别人的锁，比等一轮扫描危险。

**PID 重用**：死持有者的 pid 可能被系统复用而"看起来活着"——这仍受时间扫描约束（6h / 15min）。两条机制互补：**PID 探测抓"刚死"，时间扫描抓"复用混淆"**。

**测试手法**：不能猜一个越界 id（各平台报错不同），所以用 `spawnSync(node, ["-e", "setTimeout(...,50)"])` 拿到一个**真实存在过、已退出并被 reap** 的 pid，再探测确认 `ESRCH`——跨平台成立。探测不到就 `t.skip`，而不是硬跑。
...
| 项目 | 状态 | 改动 | 验证证据 |
|---|---|---|---|
| **🟡 #39 门间重复的助手** | ✅ | 新增 `scripts/test-file-utils.mjs`（`TEST_EXTENSIONS` / `SKIP_DIRS` / `isTestFile` / `walk` / `collectTestFiles`），耦合门与切片门改为导入；`receipt-gate` 内两份 porcelain 解析合并为一个 `porcelainPaths`（receipt 侧对"裸路径行"的容忍保留在它所属的地方） | `node --check` 4 文件全过；**16/16 + 27/27 + 12/12**；`verify:all` **14/14**；`verify:upstream` **OK**；**净 −22 行** |

**为什么排到最后仍然值得**：两个 tdd 门本应执行**同一份纪律**，却各自持有一份"什么算测试文件"的独立定义——改了一个不改另一个，两个门就开始执行不同的规则，而**没有任何机制会发现**。这也是 ADR 0007 主张（机读契约单一权威）在代码层的又一处落地。

**验证方式**：纯重构，判定标准是**行为不变**——`node --check` 四个文件、三套测试、两道全量门禁全部原样通过，且未触碰任何门禁语义。
