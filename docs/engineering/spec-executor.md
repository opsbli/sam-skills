...
The closing `SPEC EXECUTION RECEIPT` opens with a mandatory `Schema: spec-executor-receipt/v2` first field — the planning thread validates it mechanically on both the automatic and manual routes, and a missing or mismatched line is a validation failure. The receipt then reports each acceptance criterion with evidence, changed files, validation, review findings, remaining risks, final worktree state, and every external effect. It carries a mandatory `Docs delta`: every spec deviation decided in execution, new constraint discovered, or domain term used that the glossary lacks — or an explicit `none`. A blank delta is a defect, because the planning thread uses it to settle facts back into `CONTEXT.md` and ADRs before the fork is archived. The executor never edits fact documents itself. It also carries a mandatory `Receipt metrics` line — machine-parseable telemetry: the route taken (`fork` or `express`), the archive-gate outcome, grill rounds upstream, criteria evidenced, docs-delta size, and one line of `skill-friction` (`none` written explicitly when nothing snagged; a blank friction field is a defect). The planning thread harvests the metrics into `docs/metrics.md` and any non-`none` friction into `docs/skill-friction-log.md`, where the next `/project-standards audit` reviews them against the project's own standards and `/harvest run` turns repeats into skill-revision proposals. The receipt can also carry an optional `Goal / spec quality` label. The executor may leave that field blank; the planning thread or the user fills it after comparing the receipt with the actual diff. A blank label is not a failed completion.
...
No. In Codex App, [execute-spec-in-fork](https://github.com/opsbli/sam-skills/blob/main/docs/engineering/execute-spec-in-fork.md) owns the task lifecycle and uses `spec-executor` inside the child. Other harnesses can keep using the manual fork-and-receipt route.
...
- The returned receipt opens with `Schema: spec-executor-receipt/v2`, carries an explicit `skill-friction` value in its metrics line, and every acceptance criterion returns with pass/fail evidence.
...
`spec-executor` is the forked implementation step after [to-spec](https://aihero.dev/skills-to-spec). [execute-spec-in-fork](https://github.com/opsbli/sam-skills/blob/main/docs/engineering/execute-spec-in-fork.md) automates that step in Codex App, while `to-goal` remains the portable route for noisier or longer work. Use [ask-matt](https://aihero.dev/skills-ask-matt) when choosing between them.
