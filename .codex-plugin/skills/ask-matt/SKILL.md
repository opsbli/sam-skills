...
3. **Branch — how should implementation cross the context boundary?** Ask first whether it needs to cross at all. A single-file mechanical edit or an obvious fix — no open product decision, cheap validation at the seam — takes the **express lane**: do it here, in this thread, and close it with a three-line `MINI RECEIPT` (`Schema: mini-receipt/v1`, checked by `mini-receipt-gate.mjs`). That is not cutting a corner, it is choosing the right artifact: eighteen fields for a typo is misrepresentation dressed as rigour, and it is how the costly rituals get a bad name. Work above that floor continues:
   - **Small, already clear, no durable spec needed** → **`/implement`** right here, in the same context window.
   - **The approved spec fits one execution session and this thread is coherent** → **`/to-spec`**, then run **`/execute-spec-in-fork`** in Codex App. It creates a same-directory fork, launches `/spec-executor`, routes decisions back here, validates the returned receipt, and archives a clean completion. On ZCode, Claude Code, or anywhere without Codex App task tools and Codex Task Messenger, take the manual fork loop instead: stop the planning thread at the final `SPEC READY`, open a new session on the same workspace, paste the block, and run `/spec-executor` — then bring the receipt back (in ZCode you can reference the execution session as `#sess_<id>`). The full fallback runbook lives in `/execute-spec-in-fork`.
...
- **`/harvest`** — close the loop from execution evidence back to the skills. It reads the pipeline's telemetry ledgers (`docs/metrics.md`, `docs/skill-friction-log.md`) plus the eval scoreboard, drafts concrete skill-revision proposals for repeat friction or repeating quality labels, and waits for explicit human approval before anything is edited. Run it after `/project-standards audit` or when the friction log has grown — not per receipt.
