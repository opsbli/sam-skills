...
- Receipt metrics: <required, one line, machine-parseable. fork-or-express: fork|express | archive-gates: pass|fail@<gate-numbers> | archive-gate-failures: <count> | grill-rounds: <count> | criteria-evidenced: <n>/<m> | docs-delta: none|<lines> | skill-friction: none|<one line: where you stalled, which rule was ambiguous, which gate bounced - write "skill-friction: none" explicitly when there was none>>
...
`Receipt metrics` is the machine-parseable telemetry line. It reports the route this execution took (`fork-or-express`), the archive-gate outcome the planning thread will compute (`archive-gates` / `archive-gate-failures`), the interview effort upstream (`grill-rounds`), how many acceptance criteria landed with evidence (`criteria-evidenced`), the size of the `Docs delta`, and one line of `skill-friction`. The friction field is how an execution feeds back into the skills themselves: whenever a rule was ambiguous, a gate bounced a receipt the first time, a step stalled, or a skill told you two contradictory things, record it there. Write `skill-friction: none` explicitly when nothing snagged - a blank friction field is a defect, never a silent zero. The planning thread harvests this line into the repo's friction log; it is not the place to record product decisions, which belong in `Docs delta`.
