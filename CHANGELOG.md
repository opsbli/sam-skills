...
- Promote `from-prototype` to the promoted set (productivity bucket): the intake step before `grill-me` — captures a product prototype link or UI screenshots, extracts requirement facts in plain business language with every inference tagged `[待确认]`, and writes a plan draft ending in 3–6 top-level decision branches for the interview to open with. Pipeline: `from-prototype → grill-me → to-spec`. Registered across plugin.json, both READMEs, the docs page (`docs/productivity/from-prototype.md`), and the ask-matt router; codex-plugin mirror regenerated (33 skills).
- Add `fork-loop-mcp` (`scripts/fork-loop-mcp/`): a ZCode automatic-route transport for the fork loop, per ADR 0005 — an MCP service (`spawn_execution` / `check_mailbox` / `ack_receipt` / `fail_receipt` / `release_execution`) that launches the headless `zcode -p` runner on the same checkout, captures its `SPEC EXECUTION RECEIPT` into a per-checkout mailbox, and a `Stop` hook that injects the receipt into the planning session between turns (one receipt consumes one of ZCode's three Stop continuations). The single-active-execution-thread guard becomes a machine lockfile; `ack_receipt` after the six gates and telemetry harvest releases it. Planner manual steps on ZCode drop to one (`/to-spec`, then spawn). `execute-spec-in-fork` documents the route and its fallback rules.
- Roundtable-driven hardening of the fork loop (verdict 2026-09-07, recorded in ADR 0004 `.agents/adr/0004-receipt-schema-and-express-lane.md`): the `SPEC EXECUTION RECEIPT` now opens with a mandatory schema first field, validated on both the automatic and manual routes (missing or mismatched = validation failure); `execute-spec-in-fork` adds an **express lane** for work below the complexity floor (single-file mechanical edits / obvious fixes, no open product decision) that completes inline with a three-line mini receipt instead of forking; and a **single-active-execution-thread guard** on the shared checkout — before archiving, the planning thread confirms the worktree matches the receipt's reported final state and no second execution fork is running. README badge and version text corrected to `1.2.3-to-goal.3`.
